...
metadata <- read_csv("data_metagenomics/metadata.csv") %>%
...
# due to the zymo_mock or misclassification? Remove associated OGUs (G004127505, G000195995, G002032965)
...
# RCLR Transformation
species_clr <- species_sample %>% decostand(method = "rclr", impute = FALSE) %>%
  replace(is.na(.), 0)
...
# so work on them separately. AMP effect is evident. Two samples from
...
# RCLR transformation
hypo1_clr <- hypo1_samples_final %>% decostand(method = "rclr", impute = FALSE) %>%
  replace(is.na(.), 0)
...
# RCLR transformation
hypo1_amp_clr <- hypo1_amp_filter %>% decostand(method = "rclr", impute = FALSE) %>%
  replace(is.na(.), 0)
...
hypo1_dam_pre_shannon_model <- hypo1_dam_pre_alpha_info %>% 
  dplyr::filter(Day %in% c("E17", "B1", "B2")) %>%
  lmerTest::lmer(formula = Shannon ~ Amp + Day + (1|Cage))
summary(hypo1_dam_pre_shannon_model)

# Longitudinal Shannon
hypo1_shannon_long <- hypo1_dam_pre_alpha_info %>% 
  dplyr::mutate(Day = case_when(Day == "E16" ~ 1, Day == "E17" ~ 2, Day == "E18" ~ 3,
                                Day == "B1" ~ 4, Day == "B2" ~ 5, Day == "B7" ~ 10,
                                Day == "B14" ~ 17, Day == "B21" ~ 24)) %>% dplyr::mutate_at("Day", as.numeric) %>%
  ggscatter(x = "Day", y = "Shannon", color = "Amp", add = "loess", ylab = "Shannon Diversity Index",
            legend = "none", palette = c("#287DAB", "#E5BF86"), alpha = 0.5,
            title = "Alpha Diversity - Antepartum Exposure") + ylim(0,4) +
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 10, 17, 24)) + 
  theme(plot.title = element_text(size = 10),axis.title = element_text(size = 8),
        axis.text = element_text(size = 6))

hypo1_shannon_long_model <- hypo1_dam_pre_alpha_info %>% 
  dplyr::mutate(Day = case_when(Day == "E16" ~ 1, Day == "E17" ~ 2, Day == "E18" ~ 3,
                                Day == "B1" ~ 4, Day == "B2" ~ 5, Day == "B7" ~ 10,
                                Day == "B14" ~ 17, Day == "B21" ~ 24)) %>% dplyr::mutate_at("Day", as.numeric) %>%
  lmerTest::lmer(formula = Shannon ~ Amp + Day + (1|Cage))
summary(hypo1_shannon_long_model)
...
#write_csv(x = res_whole_amp_hypo1_filter_species, file = "Table_S2.csv")
...
plot_hypo1_ratio <- ratio_hypo1 %>% 
  mutate(Day = case_when(Day == "E16" ~ 1, Day == "E17" ~ 2, Day == "E18" ~ 3,
                         Day == "B1" ~ 4, Day == "B2" ~ 5, Day == "B7" ~ 10,
                         Day == "B14" ~ 17, Day == "B21" ~ 24)) %>% mutate_at("Day", as.numeric) %>%
...
# RCLR transformation
hypo2_clr <- hypo2_samples_final %>% decostand(method = "rclr", impute = FALSE) %>%
  replace(is.na(.), 0)
...
# RCLR transformation
hypo2_amp_clr <- hypo2_amp_filter %>% decostand(method = "rclr", impute = FALSE) %>%
  replace(is.na(.), 0)
...
# Sample MO_9_2_B3_Hypo2_1 must be FALSE. There is an error in the metadata (confirmed)
...
hypo2 <- hypo2 %>% dplyr::mutate(Amp = case_when(SampleID == "MO_9_2_B3_Hypo2_1" ~ "FALSE", TRUE ~ Amp))
metadata_filter <- metadata_filter %>%  dplyr::mutate(Amp = case_when(SampleID == "MO_9_2_B3_Hypo2_1" ~ "FALSE", TRUE ~ Amp))
...
hypo2_dam_pre_shannon_model <- hypo2_dam_pre_alpha_info %>% 
  dplyr::filter(Day %in% c("B2", "B3")) %>%
  lmerTest::lmer(formula = Shannon ~ Amp + Day + (1|Cage))
summary(hypo2_dam_pre_shannon_model)
...
hypo2_shannon_long_model <- hypo2_dam_pre_alpha_info %>%
  mutate(Day = case_when(Day == "B1" ~ 4, Day == "B2" ~ 5, Day == "B3" ~ 6, Day == "B7" ~ 10,
                         Day == "B14" ~ 17, Day == "B21" ~ 24)) %>% mutate_at("Day", as.numeric) %>%
  lmerTest::lmer(formula = Shannon ~ Amp + Day + (1|Cage))
summary(hypo2_shannon_long_model)
...
# Combine alpha diversity longitudinal plots from HYPO1 and HYPO2
...
#write_csv(x = res_whole_amp_hypo2_filter_species, file = "Table_S3.csv")
...
plot_hypo2_ratio <- ratio_hypo2  %>%
  dplyr::mutate(Day = case_when(Day == "B1" ~ 4, Day == "B2" ~ 5, Day == "B3" ~ 6, Day == "B7" ~ 10,
...
# Combine ratio plots from HYPO1 and HYPO2
...
# Upset plot
list_overlap <- list(
  `Antepartum PBS` = (res_whole_amp_hypo1_filter_species %>% dplyr::filter(effect < 0))$OGU,
  `Antepartum AMP` =  (res_whole_amp_hypo1_filter_species %>% dplyr::filter(effect > 0))$OGU,
...
#write_csv(x = overlap_ogus, file = "Table_S4.csv")

# Plot log ratios over time of overlapping features only
...
# ALDEx2 before FDR correction in HYPO1 and HYPO2
res_whole_amp_hypo1_fix <- res_whole_amp_hypo1 %>% 
  rownames_to_column("x") %>% dplyr::select(-x) %>%
  dplyr::rename(OGU = `rownames.ALDE_amp_hypo1_eff.`)
res_whole_amp_hypo2_fix <- res_whole_amp_hypo2 %>% 
  rownames_to_column("x") %>% dplyr::select(-x) %>%
  dplyr::rename(OGU = `rownames.ALDE_amp_hypo2_eff.`)

# Shared space
shared_features <- intersect(res_whole_amp_hypo1_fix$OGU, res_whole_amp_hypo2_fix$OGU)
N_shared <- length(shared_features)

# Filter ALDEx2 outputs to retain only the shared features
res_A_shared <- res_whole_amp_hypo1_fix %>% filter(OGU %in% shared_features)
res_B_shared <- res_whole_amp_hypo2_fix %>% filter(OGU %in% shared_features)

# BH correction is then performed on the shared subset only
res_A_shared$q_value <- p.adjust(res_A_shared$wi.ep, method = "BH")
res_B_shared$q_value <- p.adjust(res_B_shared$wi.ep, method = "BH")
...
expected_overlap <- N_shared * q_sq

# Observed overlap
significant_A <- res_A_shared$OGU[res_A_shared$q_value < fdr_threshold]
significant_B <- res_B_shared$OGU[res_B_shared$q_value < fdr_threshold]
observed_overlap <- length(intersect(significant_A, significant_B))

# Combine results
significant_hypo1 <- res_A_shared %>%
  dplyr::filter(OGU %in% intersect(significant_A, significant_B)) %>%
  dplyr::select(OGU, effect)

significant_hypo2 <- res_B_shared %>%
  dplyr::filter(OGU %in% intersect(significant_A, significant_B)) %>%
  dplyr::select(OGU, effect)

combined_sig_eff <- significant_hypo1 %>% left_join(significant_hypo2, by = "OGU") %>%
  dplyr::mutate(effect_mean = (effect.x + effect.y) / 2) %>% arrange(effect_mean) %>%
  dplyr::mutate(concordance = case_when(effect.x * effect.y > 0 ~ "Yes",
                                        TRUE ~ "No")) %>%
  left_join(lineage_table_split %>% as.data.frame() %>% rownames_to_column("OGU") %>% dplyr::select(OGU, Family, Genus, Species))

#write_csv(x = combined_sig_eff, file = "Table_S4_new.csv")


# Regenerate Upset plot
hypo1_ogu_overlap <- res_whole_amp_hypo1_fix %>% 
  dplyr::select(OGU, effect) %>% 
  dplyr::filter(OGU %in% res_whole_amp_hypo1_filter_species$OGU | OGU %in% intersect(significant_A, significant_B))
hypo2_ogu_overlap <- res_whole_amp_hypo2_fix %>% 
  dplyr::select(OGU, effect) %>% 
  dplyr::filter(OGU %in% res_whole_amp_hypo2_filter_species$OGU | OGU %in% intersect(significant_A, significant_B))

# Upset plot
list_overlap <- list(
  `Antepartum PBS` = (hypo1_ogu_overlap %>% dplyr::filter(effect < 0))$OGU,
  `Antepartum AMP` =  (hypo1_ogu_overlap %>% dplyr::filter(effect > 0))$OGU,
  `Postpartum PBS` = (hypo2_ogu_overlap %>% dplyr::filter(effect < 0))$OGU,
  `Postpartum AMP` = (hypo2_ogu_overlap %>% dplyr::filter(effect > 0))$OGU)
...
#pdf(file="Upset_metagenomics.pdf", width = 4, height = 2.5)
#importnat_upset
#dev.off()
...
# Plot log ratios over time of overlapping features only
overlap_amp_yes <- combined_sig_eff %>%  dplyr::filter(concordance == "Yes") %>%
  dplyr::filter(effect_mean > 0)
overlap_amp_no <- combined_sig_eff %>%  dplyr::filter(concordance == "Yes") %>%
  dplyr::filter(effect_mean < 0)

hypo1_overlap <- hypo1_samples_filter %>% dplyr::select(overlap_amp_yes$OGU, overlap_amp_no$OGU)
hypo2_overlap <- hypo2_samples_filter %>% dplyr::select(overlap_amp_yes$OGU, overlap_amp_no$OGU)
...
p_value_combined <- hypo_combined %>%
  ...
  dplyr::mutate(Day = case_when(Hypothesis == "1" & Day  == "E18" ~ "E17",
                                Hypothesis == "1" & Day  == "B1" ~ "B2",
                                TRUE ~ Day)) %>% group_by(Hypothesis, Day) %>% nest() %>%
  dplyr::mutate(t_test = purrr::map(data, function(df) t.test(Ratio ~ Amp, data = df)),
                p_value = purrr::map_dbl(t_test, ~.x$p.value)) %>%
  ungroup() %>%
  dplyr::mutate(p_adjust = p.adjust(p_value, method = "BH")) %>%
